document.addEventListener('DOMContentLoaded', () => {
    ...
    const Game = (() => {
        ...
        // MODIFIED: Corrected initialization order
        function init() {
            UI.init();
            Input.init();
            Renderer.init();
            resetGame(); // Initialize the game state first
            UI.populateLevelSelect(Levels); // Then populate the UI which may depend on the state
        }

        // MODIFIED: Made reset logic more robust for persistent settings
        function resetGame() {
            // Preserve settings that should persist between games (like split-screen mode)
            ...
            // Restore the persistent setting
            state.isSplitScreen = persistSplitScreen;
            
            UI.hide('dev-mode-hud');
        }

        // MODIFIED: Simplified startGame as resetGame now handles persistence
        ...
        function toggleDevMode() { state.isDevMode = !state.isDevMode; if (state.isDevMode) { UI.show('dev-mode-hud'); console.log("Dev Mode: ON"); } else { UI.hide('dev-mode-hud'); console.log("Dev Mode: OFF"); } }
        function toggleSplitScreen() { state.isSplitScreen = !state.isSplitScreen; console.log(`Split-Screen Mode: ${state.isSplitScreen ? 'ON' : 'OFF'}`); UI.updateSplitScreenButton(state.isSplitScreen); }
    ...
    const Physics = (() => {
        const C = { GRAVITY: 80, THRUST_FORCE: 400, ROTATION_SPEED: 4.5, FUEL_CONSUMPTION: 15, FUEL_REGEN: 20, DAMAGE_ON_COLLISION: 25, BOMB_STABILITY_DRAIN: 5, BOMB_STABILITY_REGEN: 3, HARMONY_ANGLE_THRESHOLD: 0.4, ROPE_LENGTH: 100, ROPE_STIFFNESS: 120, ROPE_DAMPING: 8 };
